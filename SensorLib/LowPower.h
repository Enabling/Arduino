...
enum timer5_t {
	TIMER5_OFF, TIMER5_ON
};

enum timer4_t {
	TIMER4_OFF, TIMER4_ON
};

enum timer3_t {
	TIMER3_OFF, TIMER3_ON
};

enum timer2_t {
	TIMER2_OFF, TIMER2_ON
};

enum timer1_t {
	TIMER1_OFF, TIMER1_ON
};
...
enum usart0_t {
	USART0_OFF, USART0_ON
};

enum usart1_t {
	USART1_OFF, USART1_ON
};

enum usart2_t {
	USART2_OFF, USART2_ON
};

enum usart3_t {
	USART3_OFF, USART3_ON
};
...
#error "Please ensure chosen MCU is either 168, 328P, 32U4, 1284P, 1280 or 2560."
	...
	void powerExtStandby(period_t period, adc_t adc, bod_t bod, timer2_t timer2)
			__attribute__((optimize("-O1")));
	unsigned int delayValue(period_t period) {
		switch (period) {
		case SLEEP_15MS:
			return 15;
		case SLEEP_30MS:
